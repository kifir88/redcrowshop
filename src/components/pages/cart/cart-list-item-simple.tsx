...
export default function CartListItemSimple({
    ...
    const handleIncrement = () => {
        ...
        if (data?.data) {
            if (incrementedValue <= (data?.data.stock_quantity || 0)) {
                ...
                toast.error(
                    `Товара с названием ${cartItem.name}${a ? " и аттрибутами " + attrs : ""} в наличии только ${
    ...
    {data?.data && (
        ...
        src={data?.data?.images.length > 0 ? data?.data?.images[0].src : "/category/product-image-placeholder.png"}
        alt={data?.data?.images.length > 0 ? data?.data?.images[0].src : "image-placeholder"}
        width={80}
        height={80}
        />
        </div>

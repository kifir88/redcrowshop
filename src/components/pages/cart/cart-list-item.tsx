...
export default function CartListItem({
  ...
  const updateCartItemQuantity = useCallback((item: CartItem, newQuantity: number) => {
    const updatedItems = cartItems.map(cartItem =>
      cartItem.productVariationId === item.productVariationId ? { ...cartItem, quantity: newQuantity } : cartItem
    );
  ...
  const handleIncrement = () => {
    ...
    if (data?.data) {
      if (incrementedValue <= (data?.data.stock_quantity || 0)) {
        ...
        toast.error(`Товара с названием ${cartItem.name} и аттрибутами ${cartItem.attributes.join(", ")} в наличии только ${data?.data.stock_quantity || 0}`)
      }
    }
  };
  ...
  const handleRemove = () => {
    const updatedItems = cartItems.filter(item => item.productVariationId !== cartItem.productVariationId);
    setCartItems(updatedItems);
  };

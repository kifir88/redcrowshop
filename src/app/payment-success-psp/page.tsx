...
export default async function PaymentSuccessPagePSP({
    ...
    const [strapiPageData, orderData] = await Promise.all([
        fetch(
...
function generateOrderText(order: Order | null): string {
    ...
    const { billing, shipping, line_items, total, currency } = order;
...
${line_items
        .map(
            (item) =>
                `${item.name} - Количество: ${item.quantity} - Цена: ${formatPriceToLocale(
                    item.price,
                    currency
                )}`
        )
...
<br>
Итого:<br>
${formatPriceToLocale(total, currency)}
  `.trim();
}
